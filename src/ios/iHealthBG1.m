...
// code str from ihealth ios example app
...
-(void)DeviceConnectForBG1:(NSNotification *)tempNoti{
    ...
    NSString *username = [self.commandDelegate.settings objectForKey:[@"iHealthUsername" lowercaseString]];
    NSString *clientId = [self.commandDelegate.settings objectForKey:[@"iHealthClientID" lowercaseString]];
    NSString *clientSecret = [self.commandDelegate.settings objectForKey:[@"iHealthClientSecret" lowercaseString]];

    AudioBG1Communication *bgInstance = [AudioBG1Communication audioCommunicationObject];
    if (bgInstance != nil) {

        [self sendStatus:@"connect" andKeepCallback:YES];
        NSLog(@"connect bg1");
        ...
        [bgInstance commandCreateConnectWithUserID:username clientID:clientId clientSecret:clientSecret Authentication:^(UserAuthenResult result) {
            ...
            [self sendStatus:@"ready" withData:@{@"status": [NSNumber numberWithBool:result]} andKeepCallback:YES];
